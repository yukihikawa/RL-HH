...
from src.HLS.DQN2_ERL_MORE_STATE.train.config import Config
from src.HLS.DQN2_ERL_MORE_STATE.train.replay_buffer import ReplayBuffer
...
class AgentBase:
    ...
    net_dims：MLP的中间层维度（多层感知器）

    State_dim：状态维度（状态向量的数量）

    Action_dim：动作的维度（或离散动作的数量）

    Gpu_id：训练设备的gpu_id。当cuda不可用时，请使用CPU。

    Args：代理训练的参数。`args = Config()`
    """

    def __init__(self, net_dims: [int], state_dim: int, action_dim: int, gpu_id: int = 0, args: Config = Config()):
        self.gamma = args.gamma  # discount factor of future rewards #未来奖励的折扣因子
        self.num_envs = args.num_envs  # the number of sub envs in vectorized env. `num_envs=1` in single env. 向量化环境中的子环境数。单环境中的`num_envs=1`。
        self.batch_size = args.batch_size  # num of transitions sampled from replay buffer. 从回放缓冲区中采样的转换数。
        self.if_discrete = args.if_discrete  # if the action space of env is discrete or continuous 如果环境的动作空间是离散的还是连续的
        self.repeat_times = args.repeat_times  # repeatedly update network using ReplayBuffer 中使用ReplayBuffer重复更新网络
        self.reward_scale = args.reward_scale  # an approximate target reward usually be closed to 256 or 512. 通常接近256或512的近似目标奖励。
        self.learning_rate = args.learning_rate  # the learning rate for network updating   网络更新的学习率
        self.if_off_policy = args.if_off_policy  # whether off-policy or on-policy of DRL algorithm     DRL算法是否为离线策略或在线策略
        self.clip_grad_norm = args.clip_grad_norm  # clip the gradient after normalization to avoid gradient explosion.  在规范化后剪辑梯度以避免梯度爆炸。
        self.soft_update_tau = args.soft_update_tau  # the tau of soft target update `net = (1-tau)*net + net1` 软目标更新的tau
        self.state_value_tau = args.state_value_tau  # the tau of normalize for value and state  值和状态的归一化tau
        self.state_dim = state_dim # 状态维度
        self.action_dim = action_dim # 动作维度
        self.last_state = None  # last state of the trajectory for training. last_state.shape == (num_envs, state_dim) 训练轨迹的最后一个状态。
        self.device = torch.device(f"cuda:{gpu_id}" if (torch.cuda.is_available() and (gpu_id >= 0)) else "cpu")    # 训练设备

        '''network'''
        act_class = getattr(self, "act_class", None)    # 初始化 actor网络
        cri_class = getattr(self, "cri_class", None) # 初始化 critic网络
