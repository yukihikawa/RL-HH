...
from src.HLS.TD3_ERL.env import hh_env
...
class ReplayBuffer:  # for off-policy
    ...
    def update(self, items: [Tensor]):
        ...
        print('self.p', self.p, 'p', p, 'self.max_size', self.max_size)
        if p > self.max_size:
            self.if_full = True
            p0 = self.p
            p1 = self.max_size
...
def train_sac_for_pendulum(gpu_id=0):
    ...
    args = Config(agent_class=AgentTD3, env_class=gym.make, env_args=env_args)  # see `Config` for explanation
