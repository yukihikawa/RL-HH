...
class hh_env(gym.Env):
    def __init__(self):
        ...
        # self.iter = 0
        # self.prevTime = 0
        self.heuristics = LLHolder(LLH_SET)
        # 定义动作空间
        ...
        high = np.array([len(self.heuristics), SOLVE_ITER, float('inf')])
    ...
    def termination(self):
        ...
        if self.bestTime <= IDEAL_TIME[config.PROBLEM][0] or self.ITER > SOLVE_ITER:
            return True
        else:
            return False
